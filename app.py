...
def show_tier_prediction():
    st.header("Tournament Tier Prediction Calculator")
    st.markdown("""
    Predict the appropriate tier for an upcoming tournament based on expected field strength.
    This tool analyzes player ratings to recommend a tier level based on actual skill, not geography.
    """)
    
    players = st.session_state.db.get_all_players()
    if len(players) == 0:
        st.info("Please load tournament data in the Data Management section to use tier prediction.")
        return
    
    player_data = []
    for p in players:
        conservative_rating = p.current_rating_mu - 3 * p.current_rating_sigma
        player_data.append({
            'name': p.name,
            'mu': p.current_rating_mu,
            'sigma': p.current_rating_sigma,
            'conservative': conservative_rating,
            'tournaments': p.tournaments_played
        })
    
    players_df = pd.DataFrame(player_data).sort_values('conservative', ascending=False)
    
    st.subheader("Select Expected Attendees")
    
    col1, col2 = st.columns([2, 1])
    
    with col2:
        st.markdown("**Quick Filters**")
        min_tournaments = st.slider("Minimum Tournaments Played", 0, 20, 0)
        
        if min_tournaments > 0:
            filtered_df = players_df[players_df['tournaments'] >= min_tournaments]
        else:
    ...
    with col1:
        selected_players = st.multiselect(
            "Select players expected to attend",
            options=filtered_df['name'].tolist(),
            help="Search and select players who are likely to attend this tournament"
        )
    
    if len(selected_players) < 4:
        st.info("Select at least 4 players to see tier prediction.")
        return
    
    st.divider()
    
    selected_data = players_df[players_df['name'].isin(selected_players)]
    
    field_size = len(selected_data)
    avg_mu = selected_data['mu'].mean()
    median_mu = selected_data['mu'].median()
    avg_conservative = selected_data['conservative'].mean()
    
    top_5_count = min(5, field_size)
    top_5_avg = selected_data.nlargest(top_5_count, 'conservative')['conservative'].mean()
    
    percentile_25 = selected_data['conservative'].quantile(0.25)
    percentile_75 = selected_data['conservative'].quantile(0.75)
    ...
    if composite_score >= 20:
        tier = 0
        tier_label = "Major - Championship"
        tier_color = "#FF6B00"
        explanation = f"Field strength {composite_score:.1f} indicates a championship-level tournament with the highest tier competition."
    elif composite_score >= 18:
        tier = 1
        tier_label = "Tier 1 - Elite"
        tier_color = "#FFD700"
        explanation = f"Field strength {composite_score:.1f} indicates an elite tournament with top-tier competition."
    elif composite_score >= 15:
        tier = 2
        tier_label = "Tier 2 - High Competitive"
        tier_color = "#C0C0C0"
        explanation = f"Field strength {composite_score:.1f} indicates strong competitive depth."
    elif composite_score >= 12:
        tier = 3
        tier_label = "Tier 3 - Competitive"
        tier_color = "#CD7F32"
        explanation = f"Field strength {composite_score:.1f} indicates solid competitive play."
    elif composite_score >= 9:
        tier = 4
        tier_label = "Tier 4 - Developing"
        tier_color = "#87CEEB"
        explanation = f"Field strength {composite_score:.1f} indicates developing competitive level."
    else:
        tier = 5
        tier_label = "Tier 5 - Recreational"
        tier_color = "#90EE90"
        explanation = f"Field strength {composite_score:.1f} indicates recreational level play."
    
    col1, col2, col3 = st.columns([2, 1, 1])
    
    with col1:
        st.markdown(f"""
        <div style='background-color: {tier_color}; padding: 20px; border-radius: 10px; text-align: center;'>
            <h2 style='margin: 0; color: #000;'>Recommended: {tier_label}</h2>
            <p style='margin: 5px 0 0 0; color: #000;'>{explanation}</p>
        </div>
    ...
    with col3:
        st.metric("Avg Rating", f"{avg_mu:.1f}")
        st.metric("Top 5 Avg", f"{top_5_avg:.1f}")
    
    st.divider()
    
    st.subheader("Field Strength Analysis")
    ...
    with col1:
        ...
        metrics_df = pd.DataFrame({
            'Metric': ['Average μ', 'Median μ', 'Avg Conservative', f'Top {top_5_count} Average', '25th Percentile', '75th Percentile'],
            'Value': [
                f"{avg_mu:.2f}",
                f"{median_mu:.2f}",
                f"{avg_conservative:.2f}",
                f"{top_5_avg:.2f}",
                f"{percentile_25:.2f}",
                f"{percentile_75:.2f}"
            ]
    ...
    with col2:
        ...
        fig = px.histogram(
            ...
            x='conservative',
            nbins=20,
            title='Conservative Rating Distribution',
            labels={'conservative': 'Conservative Rating (μ - 3σ)'}
        )
        st.plotly_chart(fig, use_container_width=True)
    ...
    display_df = selected_data[['name', 'mu', 'sigma', 'conservative', 'tournaments']].copy()
    display_df.columns = ['Player', 'μ (Skill)', 'σ (Uncertainty)', 'Conservative Rating', 'Tournaments']
    display_df = display_df.sort_values('Conservative Rating', ascending=False)
    display_df['Rank'] = range(1, len(display_df) + 1)
    display_df = display_df[['Rank', 'Player', 'μ (Skill)', 'σ (Uncertainty)', 'Conservative Rating', 'Tournaments']]
    
    st.dataframe(
        display_df.style.format({
            'μ (Skill)': '{:.2f}',
            'σ (Uncertainty)': '{:.2f}',
            'Conservative Rating': '{:.2f}'
        }),
        use_container_width=True,
        hide_index=True
    )
    ...
    with st.expander("How tier prediction works"):
        st.markdown("""
        **Composite Score Calculation:**
        - 40% weight: Average conservative rating (μ - 3σ) of all players
        - 30% weight: Average conservative rating of top 5 players
        - 20% weight: 75th percentile conservative rating (field depth)
        - 10% weight: Average μ (raw skill level)
        
        **Tier Thresholds:**
        - Major (Championship): Composite score ≥ 20
        - Tier 1 (Elite): 18 ≤ score < 20
        - Tier 2 (High Competitive): 15 ≤ score < 18
        - Tier 3 (Competitive): 12 ≤ score < 15
        - Tier 4 (Developing): 9 ≤ score < 12
        - Tier 5 (Recreational): score < 9
        
        **Why This Matters:**
        Traditional tier systems assign tournament levels based on geographic location or organizer status,
        not actual player skill. This tool provides an objective, data-driven tier recommendation based on
        who is actually attending, ensuring fair competition and appropriate tier assignments.
        """)
